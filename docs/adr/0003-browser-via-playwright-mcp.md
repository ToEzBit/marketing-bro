...
- Browser เป็น singleton: profile เปิดพร้อมกันสอง instance ไม่ได้ → Task ที่ขอใช้ระหว่างที่ Task อื่นถืออยู่ ~~ถูกปฏิเสธพร้อมระบุ Task ที่ถือ (ไม่เข้าคิว — ทีมเล็ก โอกาสชนต่ำ อัพเกรดเป็นคิวทีหลังได้ถ้าชนบ่อยจริง)~~ **ข้อนี้ถูกแทนที่โดย [ADR 0006](./0006-browser-queue.md): เข้าคิวรอแทนการปฏิเสธ** (การชนเกิดบ่อยจริงตามที่เผื่อไว้)
...
- ✅ ไม่มี infra เพิ่ม (ไม่มี Docker/service) — `@playwright/mcp` จาก dependency ที่ pin ไว้รันเป็น MCP server ของ session (resolve ตรงจาก node_modules ไม่ผ่าน `npx` เพื่อให้เวอร์ชันนิ่งและ `playwright-core` เป็นสำเนาเดียวกันตามข้อบังคับด้านบน)
