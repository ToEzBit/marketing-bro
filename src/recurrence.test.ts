...
check("daily at 08:00 created at 07:00 fires the same morning", () => {
  const rec: Recurrence = { kind: "clock", hour: 8, minute: 0, everyDays: 1 };
...
check("every 3 days counts from the creation date", () => {
  const rec: Recurrence = { kind: "clock", hour: 21, minute: 0, everyDays: 3 };
...
check("mon/wed at 09:00 from a Friday goes to Monday", () => {
  const rec: Recurrence = { kind: "clock", hour: 9, minute: 0, days: [1, 3] };
...
check("fires later the same day when the time is still ahead", () => {
  const rec: Recurrence = { kind: "clock", hour: 9, minute: 0, days: [5] }; // Friday
...
check("interval, daily, every-N-days, weekdays", () => {
  ...
  assert.equal(
    describeRecurrence({ kind: "clock", hour: 8, minute: 0, everyDays: 1 }),
    "ทุกวัน เวลา 08:00",
  );
  assert.equal(
    describeRecurrence({ kind: "clock", hour: 21, minute: 30, everyDays: 3 }),
    "ทุก 3 วัน เวลา 21:30",
  );
  assert.equal(
    describeRecurrence({ kind: "clock", hour: 9, minute: 0, days: [1, 3] }),
